...
// menu store stays open; selection is dispatched through the same scoped
// slash/input-insert-reference event the pipeline uses, with the controller's
// live hit span (draft-rev CAS intact). On submit the source codec serializes
// each occurrence to the model-visible form `<file>src/main.ts</file>`.
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var CAP_3 = 200;
    var CAP_5 = 500;
    /** Longest lexicon kept per session (chip decoration of hand-typed paths). */
    var LEXICON_CAP = 3000;
    ...
    /** Per-session lexicon rolls for chip decoration (fed by successful fetches). */
    function LexiconState() {
      this.sets = new Map();
      this.listeners = new Map();
    }
    LexiconState.prototype.feed = function (sessionId, files) {
      if (!Array.isArray(files) || files.length === 0) return;
      var set = this.sets.get(sessionId);
      if (set === void 0) {
        set = new Set();
        this.sets.set(sessionId, set);
      }
      var grew = false;
      for (var i = 0; i < files.length && set.size < LEXICON_CAP; i++) {
        if (!set.has(files[i].path)) {
          set.add(files[i].path);
          grew = true;
        }
      }
      if (!grew) return;
      var ls = this.listeners.get(sessionId);
      if (!ls) return;
      for (var j = 0; j < ls.length; j++) {
        try { ls[j](); } catch (error) { console.error("[dsh-input-file-ref] lexicon listener failed:", error); }
      }
    };
    LexiconState.prototype.roll = function (sessionId) {
      var set = this.sets.get(sessionId);
      return set === void 0 || set.size === 0 ? void 0 : Array.from(set.keys());
    };
    LexiconState.prototype.subscribe = function (sessionId, listener) {
      var ls = this.listeners.get(sessionId) || [];
      ls.push(listener);
      this.listeners.set(sessionId, ls);
      var self = this;
      return function () {
        var cur = self.listeners.get(sessionId) || [];
        var idx = cur.indexOf(listener);
        if (idx >= 0) {
          cur.splice(idx, 1);
          if (cur.length === 0) self.listeners.delete(sessionId);
          else self.listeners.set(sessionId, cur);
        }
      };
    };
        ...
        var run = function () {
          ...
          fetch("/api/input-file-list?" + parts, { signal: sup.signal, headers: { accept: "application/json" } })
            ...
            .then(function (payload) {
              ...
              lexicon.feed(sessionId, payload && payload.files);
            })
      ...
      function insertFile(ref) {
        ...
        try {
          applied = actx.bail(actx, "slash/input-insert-reference", {
            reference: { source: "file", ref: ref, label: "@" + ref, clipboardText: "@" + ref },
            span: span
          }) === true;
        } catch (error) {
    ...
    function apply(ctx) {
      ...
      var source = {
        ...
        lexicon: function (session) {
          return lexicon.roll(session.sessionId);
        },
        subscribeLexicon: function (session, listener) {
          return lexicon.subscribe(session.sessionId, listener);
        },
        codec: {
