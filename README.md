...
dsh-input-file-ref (node half, lib/index.js — 零外部依赖，仅 node 内置)
  ...
  文件选择面板 (client half, conversation.input.overlay) ── 顶部搜索栏 + 逐级下钻
    ...
    └─ 选中 → 相对全路径回填进输入框（前后各一空格）──发送──► 模型读取该文件
```
...
- ✅ 选中后，文件**相对全路径**以普通文本形式回填到输入框（前后各一个空格，可直接编辑、删除、多引用多个文件）。
...
6. 用 ↑/↓ + Enter（或鼠标点击 `📄` 文件行）选择文件 → 文件的**相对全路径**以普通文本回填进输入框（前后各一个空格），例如 ` src/main.ts `。
7. 发送后，消息中带有所引用的相对路径，模型可用 fs 工具按该路径读取内容。
8. 删除/修改：回填的是普通文本，直接当作普通文字编辑或删除即可；可连续引用多个文件（每个前后各一空格分隔）。
...
- **Q：可以引用多个文件吗？** A：可以。重复打开 `@` 面板选择，每次回填一个相对路径（前后各一空格），可连续引用多个。
